...
6/7 房间已布置，唯一未布置空间为停车区；它仍是首套质量标杆，不代表现代/北欧已拥有独立
厨卫模型材质变体，也不等同于离线照片级最终效果图。

在此基础上，卧室 1 已升级为“商业级实时卧室”候选基准：床体采用可垂坠床品几何、软包分缝、
四枕与腰枕组合，并嵌入可审计的 512 px 织物 PBR；床头增加项目自有轻量浮雕画，床侧吊灯和
床尾近景形成完整视觉焦点。相同床模型可在暖木、北欧和现代三套风格间保留纹理换色。该基准
等待产品验收冻结后，才会按同一标准批量升级其余卧室、厨房和卫生间。
...
3. 在 layout v6 之上实现家具拖拽、约束反馈、布局保存/恢复和任意旋转家具约束。
