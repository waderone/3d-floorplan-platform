...
鸟瞰+客厅+卧室多视角”，以及“户型图→墙/房间候选→置信度叠加图→人工复核”
技术闸门均已跑通。下一批工作是：

1. 建立合法的真实户型评测/训练集，升级斜墙、门窗、文字干扰和房间语义模型。
2. 把识别建议叠加到编辑器，完成逐条接受/拒绝/修正后再写入权威场景的 UI 闭环。
...
下一阶段将识别 `review_required` manifest 接入编辑器叠加层，先完成墙/房间建议的
逐条人工确认和场景版本写入；同时收集并标注可商用真实户型图，为门窗、斜墙和房间语义模型建立可量化基线。
