...
layout v6 从 Pascal 权威 Door/Window 派生门扇开启区和窗边接近区。家具只在通过边界、互碰
和开口净空后发布；页面显示门窗数量和阻断状态，并提供默认关闭的 `⌗` 动线检查层。曲墙或
宿主不完整的开口会显式计入 ignored，不由 Viewer 猜测。
完整卧室组合无法落入紧凑房间时，v6 会确定性降级为真实床、单侧床头柜/台灯、地毯和吊灯，
并继续满足房间边界、墙体净空与门洞净空；不会为追求数量把床放到墙外或门扇开启区。

卧室、厨房和卫生间使用各自的主体近景构图。进入房间近景时隐藏相邻房间家具并降低建筑遮挡；
暖木厨房增加石材地面，卫生间增加浅石地砖与勾缝，局部灯光突出床头、操作台和盥洗区。
卧室近景以床尾侧为主视角并完整保留床品、床头板、床侧吊灯和浮雕画；宽屏半径保持
3.8～4.1 m，避免界面覆盖或近裁剪把床尾切掉。
