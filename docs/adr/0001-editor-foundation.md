...
8. 当前发布流程没有完整发布内置 nodes，`editor` 包也直接导出源码，单纯 npm 包依赖尚不足以复现完整应用。因此开源基线继续以公开上游 submodule 固定完整应用，产品集成改动需要在本仓库公开维护。
9. 本轮只证明底座可以安装、构建和运行；完整的户型标定编辑闭环、跨端资源恢复、GLB 优化及手机性能仍属于后续技术闸门。

## 当前结论

Pascal Editor 满足 Phase 0 的候选底座标准。开源发行版将 submodule 固定到可公开克隆的上游 commit，并把资源上传、FastAPI 接入和导出钩子作为后续公开集成工作；不再要求访问私有产品化镜像。FastAPI、S3/MinIO、业务权限和场景 envelope 尽量留在产品宿主，控制上游改动面。

## 后续决策

ADR-0002 已确认采用第 1 种方案。其余方案保留为本阶段的历史备选：

1. 维护固定版本的公开产品化 fork，或向上游贡献通用扩展点（当前推荐）。
