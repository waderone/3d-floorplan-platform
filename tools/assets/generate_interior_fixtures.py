...
GENERATOR_VERSION = "interior-fixtures-v1"


def parse_arguments() -> argparse.Namespace:
    values = sys.argv[sys.argv.index("--") + 1 :] if "--" in sys.argv else []
    parser = argparse.ArgumentParser(description="Generate project-owned interior fixture GLBs")
    parser.add_argument("--asset", required=True, choices=("kitchen", "bathroom", "planter"))
...
def build_kitchen() -> tuple[str, list[bpy.types.Object]]:
    ...
    oak = material("natural-oak", (0.43, 0.24, 0.12, 1), 0.48)
    ivory = material("warm-ivory-cabinet", (0.78, 0.72, 0.62, 1), 0.66)
    stone = material("travertine-worktop", (0.63, 0.55, 0.44, 1), 0.34)
    graphite = material("graphite-appliance", (0.025, 0.028, 0.032, 1), 0.24, 0.5)
    steel = material("brushed-steel", (0.38, 0.4, 0.42, 1), 0.2, 0.8)
    ...
    objects.extend(
        [
            ...
            rounded_box("wall-cabinet-left", (0.86, 0.36, 0.72), (-0.49, 0.13, 1.72), ivory, 0.035),
            rounded_box("wall-cabinet-right", (0.86, 0.36, 0.72), (0.49, 0.13, 1.72), ivory, 0.035),
        ]
    )
    for index, x in enumerate((-0.68, -0.23, 0.23, 0.68), start=1):
    ...
    for index, x in enumerate((-0.49, 0.49), start=1):
        objects.append(rounded_box(f"wall-door-{index}", (0.81, 0.03, 0.66), (x, -0.065, 1.72), ivory, 0.018))
        objects.append(cylinder(f"wall-handle-{index}", 0.011, 0.25, (x + (-0.28 if x > 0 else 0.28), -0.09, 1.67), steel))
    ...
    objects.append(rounded_box("under-cabinet-light", (1.62, 0.045, 0.025), (0, -0.08, 1.34), steel, 0.008))
...
def build_bathroom() -> tuple[str, list[bpy.types.Object]]:
    ...
    graphite = material("bathroom-graphite", (0.035, 0.04, 0.045, 1), 0.22, 0.65)
    glass = material("shower-glass", (0.54, 0.68, 0.7, 1), 0.1, alpha=0.28)
    objects: list[bpy.types.Object] = []
    ...
    objects.extend(
        [
            ...
            rounded_box("mirror", (0.68, 0.045, 0.76), (-0.38, 0.24, 1.45), glass, 0.03),
            rounded_box("mirror-frame", (0.75, 0.035, 0.83), (-0.38, 0.27, 1.45), graphite, 0.035),
            rounded_box("mirror-face", (0.68, 0.02, 0.76), (-0.38, 0.245, 1.45), glass, 0.025),
